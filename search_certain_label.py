...
def find_files_with_label_4(folder_path):
    """
    지정된 폴더 내의 모든 라벨 파일을 검사하여,
    파일 내용 중 라벨(클래스 번호, 각 줄의 첫 번째 숫자)이
    '4'인 경우가 하나라도 포함된 파일들의 이름을 리스트로 반환합니다.
    """
    files_with_label_4 = []
    ...
    if not os.path.exists(folder_path):
        ...
        return files_with_label_4 # 빈 리스트 반환
    ...
    # 2. 폴더 내 모든 파일에 대해 처리
    print(f"'{folder_path}' 폴더를 검사 중입니다 (라벨 '4'를 찾는 중)...")
    ...
    for filename in os.listdir(folder_path):
        ...
        if os.path.isfile(file_path) and filename.lower().endswith('.txt'):
            ...
            try:
                ...
                found_label_4_in_file = False # 현재 파일에서 라벨 '4'를 찾았는지 여부
                for line_number, line in enumerate(lines, 1): # 줄 번호도 함께 확인 (디버깅용)
                    parts = line.strip().split()
                    if parts: # 빈 줄이 아닌 경우에만 처리
                        label_class = parts[0]
                        # 첫 번째 부분이 숫자이고, '4'인지 확인
                        if label_class.isdigit() and label_class == '4':
                            files_with_label_4.append(filename)
                            found_target_files_count +=1
                            # print(f"  -> 파일 '{filename}'의 {line_number}번째 줄에서 라벨 '4' 발견.")
                            found_label_4_in_file = True
    ...
    return files_with_label_4

# --- ★★★★★ 사용자 설정 부분 ★★★★★ ---
# 라벨 파일들이 있는 폴더 경로를 아래 변수에 직접 할당하세요.
# Linux/macOS 예시:
label_folder_path = '/home/ivis/anaconda3/envs/yolov9/yolov9/data/train/labels' 
# Windows 예시:
# label_folder_path = r'C:\Users\YourUser\Desktop\my_dataset\labels' # Windows 경로는 r'' 또는 '\\' 사용

# --- 실행 부분 ---
if label_folder_path: # 변수에 경로가 할당되었는지 확인
    result_files = find_files_with_label_4(label_folder_path)

    if result_files:
        print(f"\n총 {len(result_files)}개의 파일에서 라벨 '4'가 발견되었습니다:")
        for fname in result_files:
            print(fname)
    elif os.path.exists(label_folder_path) and os.path.isdir(label_folder_path): # 폴더는 존재하지만 해당 파일이 없는 경우
        print(f"\n'{label_folder_path}' 폴더 내에 라벨 '4'를 포함하는 파일을 찾지 못했습니다.")
    # 폴더가 존재하지 않거나 폴더가 아닌 경우는 find_files_with_label_4 함수 내에서 이미 메시지 출력됨
